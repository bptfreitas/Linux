...
[[ ! -d $MOUNT_POINT ]] &&\
    { echo "Invalid mount point: $MOUNT_POINT" ; exit -1; }

mount -l | egrep "${DEVICE}" | egrep -q "${MOUNT_POINT}"

[[ $? -ne 0 ]] &&\
    { echo "${DEVICE} not mounted on ${MOUNT_POINT}" ; exit -1; } 

[[ ! -w "${MOUNT_POINT}" ]] &&\
    { echo "Can't write in ${MOUNT_POINT}"; exit -1; }

fsSzB4_sm_wr_bytes="`du -sh ${MOUNT_POINT} 2> /dev/null | awk '{ print $1 }'`"
fsSzB4_sm_wr_blocks="`du -s ${MOUNT_POINT} 2> /dev/null | awk '{ print $1 }'`"
...
fsSzAfter_sm_wr_bytes="`du -sh ${MOUNT_POINT} 2> /dev/null | awk '{ print $1 }'`"
fsSzAfter_sm_wr_blocks="`du -s ${MOUNT_POINT} 2> /dev/null | awk '{ print $1 }'`"
...
fsSzAfter_sm_del_bytes="`du -sh ${MOUNT_POINT} 2> /dev/null | awk '{ print $1 }'`"
fsSzAfter_sm_del_blocks="`du -s ${MOUNT_POINT} 2> /dev/null | awk '{ print $1 }'`"
...
fio --name=random-write \
    ...
    --runtime=60 \
    --time_based \
    --end_fsync=1 > fs_randwrite_1job_4Gfs_4Kblocks.dat
...
fio --name=random-write \
    ...
    --runtime=60 \
    --time_based \
    --end_fsync=1 > fs_randwrite_16job_256Mfs_64Kblocks.dat
...
fio --name=random-write \
    ...
    --runtime=60 \
    --time_based \
    --end_fsync=1 > fs_randwrite_1job_1Gfs_1Mblocks.dat